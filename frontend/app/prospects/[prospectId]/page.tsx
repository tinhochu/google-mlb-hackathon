...
import { getUserByClerkId } from '@/db/queries/select'
...
export default async function ProspectPage({
  ...
  if (user) {
    const users = await getUserByClerkId(user?.id as string)
    dbUser = users[0] || null // Get the first user or null if the array is empty
  }
