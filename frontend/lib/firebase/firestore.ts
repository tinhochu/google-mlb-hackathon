...
export async function createOrUpdateProspect(prospect: Prospect) {
  // check if prospect exists
  const prospectRef = doc(collection(db, 'prospects'), prospect.id)
  const prospectDoc = await getDoc(prospectRef)
  if (prospectDoc.exists()) {
    await updateDoc(prospectRef, prospect)
  } else {
    await setDoc(prospectRef, prospect)
  }
}

export async function getProspectById(db: Firestore, prospectId: string) {
  try {
    const prospectRef = doc(collection(db, 'prospects'), prospectId)
    const prospectDoc = await getDoc(prospectRef)
    return prospectDoc.data()
  } catch (error) {
