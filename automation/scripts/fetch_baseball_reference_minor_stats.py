...
def fetch_player_stats(players):
    ...
    def fetch_stat(url, index):
        """Fetch player stats from Baseball-Reference using the provided URL."""
        try:
        ...
        for section in ['p1', 'p2', 'p3']:
            ...
            if div:
                for stat_div in div.find_all('div'):
                    ...
                    stats[key] = float(value) if value.replace('.', '', 1).isdigit() else 0
        return stats
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Scraping URLs"):
            ...
            if stats_pullout:
                ...
                for key, value in stats.items():
                    updated_players.at[index, key] = int(value) if isinstance(value, float) else value
...
def initialize_player_stats(data):
    ...
    stat_columns = {
        # Pitchers
        'Career_WAR': 0, 'Career_W': 0, 'Career_L': 0, 'Career_ERA': 0.0,
        'Career_G': 0, 'Career_GS': 0, 'Career_SV': 0, 'Career_IP': 0.0,
        'Career_SO': 0, 'Career_WHIP': 0.0,
        # Hitters
        'Career_AB': 0, 'Career_HR': 0, 'Career_BA': 0.0, 'Career_R': 0,
        'Career_RBI': 0, 'Career_SB': 0, 'Career_OBP': 0.0, 'Career_SLG': 0.0,
        'Career_OPS': 0.0, 'Career_OPS+': 0
    }
...
def main():
    ...
    updated_data = fetch_player_stats(initialized_data)
